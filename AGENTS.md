...
- **SQLite** — data layer via stdlib `sqlite3`; database at `data/delivery.db` (override with `DB_PATH` env var); gitignored — seeded automatically by `demo.sh`, or manually with `uv run python seed.py`
